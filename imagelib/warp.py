...
def gen_warp_params(source, flip, rotation_range=[-10, 10], scale_range=[-0.5, 0.5], tx_range=[-0.05, 0.05], ty_range=[-0.05, 0.05]):
    ...
    if (h != w) or (w != 64 and w != 128 and w != 256 and w != 512 and w != 1024):
        raise ValueError(
            'TrainingDataGenerator accepts only square power of 2 images.')
...
def warp_by_params(params, img, warp, transform, flip, is_border_replicate):
    ...
    if transform:
        img = cv2.warpAffine(img, params['rmat'], (params['w'], params['w']), borderMode=(
            cv2.BORDER_REPLICATE if is_border_replicate else cv2.BORDER_CONSTANT), flags=cv2.INTER_CUBIC)
